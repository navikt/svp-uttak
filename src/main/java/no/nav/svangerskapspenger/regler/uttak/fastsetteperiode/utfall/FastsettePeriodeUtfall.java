...
import no.nav.svangerskapspenger.regler.uttak.FastsettePeriodePropertyType;
...
public class FastsettePeriodeUtfall extends LeafSpecification<FastsettePeriodeGrunnlag> {
    ...
    private FastsettePeriodeUtfall(String id, PeriodeÅrsak periodeÅrsak, UtfallType utfallType) {
        ...
        this.utfallSpesifiserere.add((singleEvaluation, grunnlag) -> {
            singleEvaluation.getEvaluationProperties().put(FastsettePeriodePropertyType.UTFALL, utfallType);
            singleEvaluation.getEvaluationProperties().put(FastsettePeriodePropertyType.ÅRSAK, periodeÅrsak);
        });
    }
